...
export class FundController {
  ...
  async getFund(req: AuthenticatedRequest, res: Response) {
    try {
      const userId = req.user?.id;
      const { id } = req.params;
  ...
  async createFund(req: AuthenticatedRequest, res: Response) {
    try {
      const userId = req.user?.id;
      ...
      // Create fund
      const result = await query(
        `INSERT INTO funds (name, description, target_amount, cover_image_url, owner_id, deadline)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id`,
        [name, description, targetAmount, coverImageUrl, userId, deadline]
      );
      ...
      // Add owner as member
      await query(
        `INSERT INTO fund_members (fund_id, user_id, role) VALUES ($1, $2, 'owner')`,
        [fundId, userId]
      );
  ...
  async updateFund(req: AuthenticatedRequest, res: Response) {
    try {
      const userId = req.user?.id;
      const { id } = req.params;
      const { name, description, targetAmount, coverImageUrl, deadline } = req.body;
      ...
      const result = await query(
        `UPDATE funds SET
         ...
         RETURNING id`,
        [name, description, targetAmount, coverImageUrl, deadline, id, userId]
      );
  ...
  async deleteFund(req: AuthenticatedRequest, res: Response) {
    try {
      const userId = req.user?.id;
      const { id } = req.params;

      await query('DELETE FROM funds WHERE id = $1 AND owner_id = $2', [id, userId]);
  ...
  async contribute(req: AuthenticatedRequest, res: Response) {
    try {
      const userId = req.user?.id;
      const { id } = req.params;
      ...
      await query(
        ...
        [id, userId, amount, note]
      );

      // Update fund amount
      await query(
        'UPDATE funds SET current_amount = current_amount + $1 WHERE id = $2',
        [amount, id]
      );

      // Update member contribution
      await query(
        'UPDATE fund_members SET contribution = contribution + $1 WHERE fund_id = $2 AND user_id = $3',
        [amount, id, userId]
      );
  ...
  async getContributions(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const result = await query(
        `SELECT fc.*, u.name, u.email, u.avatar_url
  ...
  async inviteMember(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      // Add as member
      await query(
        `INSERT INTO fund_members (fund_id, user_id, role) VALUES ($1, $2, $3)
         ON CONFLICT (fund_id, user_id) DO NOTHING`,
        [id, invitedUserId, role]
      );
  ...
  async removeMember(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      await query(
        'DELETE FROM fund_members WHERE fund_id = $1 AND user_id = $2 AND role != $3',
        [id, targetUserId, 'owner']
      );
