...
export class UserController {
  ...
  async googleOAuth(req: Request, res: Response) {
    try {
      ...
      if (oauthResult.rows.length > 0) {
        ...
        const existingUser = await query('SELECT id, email, name, phone, role FROM users WHERE email = $1', [email]);
  ...
  async deleteAccount(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const anonymizedEmail = `deleted_${uuidv4()}@deleted.local`;
      await query(
        `UPDATE users SET 
           email = $1,
           name = 'Deleted User',
      ...
      await query('UPDATE email_verification_tokens SET verified = true WHERE user_id = $1', [userId]);
