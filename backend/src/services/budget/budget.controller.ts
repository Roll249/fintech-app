...
export class BudgetController {
  ...
  async createBudget(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const result = await query(
        `INSERT INTO budgets (user_id, category_id, amount_limit, spent, period, start_date, end_date, alert_threshold)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
         RETURNING id`,
        [userId, categoryId, limit, initialSpent, period, startDate, endDate, alertThreshold]
      );
  ...
  async resetBudget(req: AuthenticatedRequest, res: Response) {
    try {
      const userId = req.user?.id;
      const { id } = req.params;

      const result = await query(
        `UPDATE budgets SET spent = 0
         WHERE id = $1 AND user_id = $2
         RETURNING id, amount_limit`,
        [id, userId]
      );
