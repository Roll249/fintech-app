...
export class BankService {
  ...
  async processWebhookPayload(bankCode: string, payload: WebhookPayload): Promise<{ processed: boolean; message: string }> {
    const { eventId, eventType, accountNumber, balance, transactions } = payload;

    // Check for duplicate event (idempotency)
    const existingEvent = await query(
      'SELECT id FROM bank_webhook_events WHERE event_id = $1 AND bank_code = $2',
      [eventId, bankCode]
    );
    ...
    await query(
      `INSERT INTO bank_webhook_events (id, bank_code, event_id, event_type, payload, created_at)
       VALUES ($1, $2, $3, $4, $5, NOW())`,
      [uuidv4(), bankCode, eventId, eventType, JSON.stringify(payload)]
    );

    // Find linked account by account number and bank code
    const accountResult = await query(
      'SELECT id, user_id FROM accounts WHERE account_number = $1 AND bank_code = $2',
    ...
    await query(
      'UPDATE bank_webhook_events SET processed_at = NOW(), status = $1 WHERE event_id = $2 AND bank_code = $3',
      ['processed', eventId, bankCode]
    );

    return { processed: true, message: 'Webhook processed successfully' };
  }

  async createTransactionsFromBank(
    accountId: string,
    userId: string,
    transactions: BankTransaction[]
  ): Promise<number> {
    ...
    for (const tx of transactions) {
      ...
      const existingTx = await query(
        'SELECT id FROM transactions WHERE external_id = $1 AND account_id = $2',
        [tx.externalId, accountId]
      );
  ...
  async syncAccountBalance(accountId: string, balance: number): Promise<void> {
    await query(
  ...
  private generateOAuthState(accountId: string): string {
    const payload = {
      accountId,
      timestamp: Date.now(),
      nonce: uuidv4(),
  ...
  verifyOAuthState(state: string): { valid: boolean; accountId?: string } {
    try {
  ...
  private getBankClientId(bankCode: string): string {
  ...
  async getConnectionStatus(accountId: string, userId: string): Promise<{
    connected: boolean;
    status: string;
    ...
    const result = await query(
      `SELECT connection_status, last_synced_at, bank_code, bank_token_expires_at
       FROM accounts
