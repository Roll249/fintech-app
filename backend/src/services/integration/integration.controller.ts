...
import { BankService, SUPPORTED_BANKS, WebhookPayload } from './bank.service.js';
...
export class IntegrationController {
   ...
   */
  async handleBankWebhook(req: WebhookRequest, res: Response) {
    try {
   ...
   */
  async initBankOAuth(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const oauthUrl = bankService.generateOAuthUrl(
        bankCode,
        accountId || uuidv4(), // Use temp ID if no account yet
        callbackUrl
      );
   ...
   */
  async handleBankOAuthCallback(req: Request, res: Response) {
    try {
      const { bankCode } = req.params;
      ...
      // Store tokens
      await bankService.storeOAuthTokens(
        accountId!,
        bankCode,
      ...
      await query(
        'UPDATE accounts SET bank_code = $1, updated_at = NOW() WHERE id = $2 AND bank_code IS NULL',
        [bankCode, accountId]
      );
   ...
   */
  private async exchangeCodeForTokens(bankCode: string, code: string): Promise<{
    access_token: string;
   ...
   */
  async getBankConnectionStatus(req: AuthenticatedRequest, res: Response) {
    try {
      const userId = req.user?.id;
      const { bankCode } = req.params;
