...
async function logAudit(userId, action, resourceType, resourceId, oldValue, newValue, req) {
    ...
    await (0, db_js_1.query)(`INSERT INTO audit_logs (user_id, action, resource_type, resource_id, old_value, new_value, ip_address, user_agent)
...
class AdminController {
    async getUsers(req, res) {
      ...
      `;
            ...
            // Get total count
            const countResult = await (0, db_js_1.query)(queryStr.replace('SELECT id, email, name, phone, role, status, avatar_url, created_at, updated_at', 'SELECT COUNT(*)'), params);
    ...
    async getUser(req, res) {
        try {
            const { id } = req.params;
            const userResult = await (0, db_js_1.query)(`SELECT id, email, name, phone, role, status, avatar_url, created_at, updated_at
         FROM users WHERE id = $1`, [id]);
           ...
           (SELECT COUNT(*) FROM bills WHERE user_id = $1) as bill_count`, [id]);
    ...
    async updateUserStatus(req, res) {
        try {
            ...
            const currentResult = await (0, db_js_1.query)('SELECT status, email, name FROM users WHERE id = $1', [id]);
            if (currentResult.rows.length === 0) {
                return res.status(404).json({ error: 'User not found' });
            }
            const oldStatus = currentResult.rows[0].status;
            // Update status
            await (0, db_js_1.query)('UPDATE users SET status = $1, updated_at = NOW() WHERE id = $2', [status, id]);
            // Log audit
            await logAudit(adminId, status === 'locked' ? 'USER_LOCKED' : 'USER_UNLOCKED', 'user', id, { status: oldStatus }, { status, reason }, req);
    ...
    async resetUserPassword(req, res) {
        try {
            ...
            const userResult = await (0, db_js_1.query)('SELECT email, name FROM users WHERE id = $1', [id]);
            if (userResult.rows.length === 0) {
                return res.status(404).json({ error: 'User not found' });
            }
            ...
            // Update password
            await (0, db_js_1.query)('UPDATE users SET password_hash = $1, updated_at = NOW() WHERE id = $2', [passwordHash, id]);
            // Revoke all refresh tokens for this user
            await (0, db_js_1.query)('UPDATE refresh_tokens SET revoked = true WHERE user_id = $1', [id]);
            // Log audit
            await logAudit(adminId, 'PASSWORD_RESET', 'user', id, null, { resetBy: adminId }, req);
            res.json({
    ...
    async getDashboard(req, res) {
        try {
            // Get total users
            const usersResult = await (0, db_js_1.query)(`
        SELECT 
          COUNT(*) as total,
      ...
      `);
            ...
            const transactionsResult = await (0, db_js_1.query)(`
        SELECT 
          COUNT(*) as total,
      ...
      `);
            // Get bill stats
            const billsResult = await (0, db_js_1.query)(`
        SELECT 
          COUNT(*) as total,
      ...
      `);
            // Get OCR stats
            const ocrResult = await (0, db_js_1.query)(`
        SELECT 
          COUNT(*) as total,
    ...
    async getOcrStats(req, res) {
        try {
            ...
            // Get daily OCR stats
            const dailyResult = await (0, db_js_1.query)(`SELECT 
         ...
         LIMIT 30`, params);
            ...
            const overallResult = await (0, db_js_1.query)(`SELECT 
           COUNT(*) FILTER (WHERE ocr_status = 'success') as success,
           COUNT(*) FILTER (WHERE ocr_status = 'failed') as failed,
           COUNT(*) as total,
           AVG(CASE WHEN ocr_status = 'success' THEN 1 ELSE 0 END) * 100 as success_rate
         FROM bills 
         WHERE ocr_status IS NOT NULL ${dateFilter}`, params);
    ...
    async broadcastNotification(req, res) {
        try {
            ...
            const usersResult = await (0, db_js_1.query)("SELECT id FROM users WHERE status = 'active'");
            ...
            await (0, db_js_1.query)(`INSERT INTO notifications (user_id, type, title, body, data) VALUES ${values}`, params);
            // Log audit
            await logAudit(adminId, 'BROADCAST_NOTIFICATION', 'notification', null, null, { title, body, type, recipientCount: userIds.length }, req);
            res.json({
    ...
    async getFunds(req, res) {
      ...
      `;
            ...
            // Get total count
            const countQuery = queryStr.replace(/SELECT f\.\*, u\.name as owner_name.*member_count/, 'SELECT COUNT(*)');
            const countResult = await (0, db_js_1.query)(countQuery, params);
            const total = parseInt(countResult.rows[0].count);
            // Add pagination
    ...
    async getAuditLogs(req, res) {
        try {
            const { page = 1, pageSize = 20, userId, action, resourceType, startDate, endDate, } = req.query;
            const offset = (Number(page) - 1) * Number(pageSize);
            let queryStr = `
      ...
      `;
            ...
            // Get total count
            const countQuery = queryStr.replace(/SELECT al\.\*, u\.name as user_name, u\.email as user_email/, 'SELECT COUNT(*)');
            const countResult = await (0, db_js_1.query)(countQuery, params);
            const total = parseInt(countResult.rows[0].count);
            // Add pagination
